...
\newpage
\subsubsection*{The Bohr Model of the H atom}
\formulaListStart
...
\subsubsection*{Math Basis for Uncertainty}
\formulaListStart
...
\formula{$\int_{-\infty}^{+\infty} e^{-az^2+bz}\,dz = \sqrt{\frac{\pi}{a}}e^{\frac{b^2}{4a}}$}{Gaussian Integral.}
\formulaListEnd

\subsubsection*{Stationary States}
\formulaListStart
\formula{$-\frac{\hbar^2}{2m} \frac{\partial^2}{\partial x^2}\psi(x) + U(x)\psi(x)= E\psi(x)$}{Time-independent Schrodinger Equation.}
\formula{$\Psi(x,t) = \psi(x)e^{-iEt/\hbar}$}{Total wave function relation to the temporal part.}
\formulaListEnd
\end{document}
